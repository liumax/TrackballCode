...
[auxFileName] = functionFileFinder(subFoldersCell,'DIO','Aux1');
if length(auxFileName) == 0
    [auxFileName] = functionFileFinder(subFoldersCell,'DIO','Aux1');
end
auxFileName = auxFileName{1};
%extracts DIO stuffs! this code is written to extract inputs for d1
[cameraFrameDIO] = readTrodesExtractedDataFile(auxFileName);

% %pulls out DIO up state onsets.
% [cameraFrameDIOTimes,cameraFrameDIODiff] = functionBasicDIOCheck(cameraFrameDIO,s.Parameters.trodesFS);
...
dioTimes = double(cameraFrameDIO.fields(1).data)/s.Parameters.trodesFS;
%extracts states. States should be 0 or 1.
dioState = double(cameraFrameDIO.fields(2).data);
...
cameraFrameDIOTimes =dioTimes;
length(cameraFrameDIOTimes)
...
%     bigHistEDR(:,i) = histEDR; %for plotting purposes!
    ...
    histLocoStart = histCounts'/length(s.RotaryData.PosStartsEnds(:,1))/s.Parameters.histBin;
...
%     edrEnd = edrMagTimes(find(~isnan(edrMagTimes),1,'last'));
    ...
    if toggleROC == 1
        ...
        title('Normalized Vel (k) and Firing Rate (r) 10ms bin 300ms smooth')
    end
