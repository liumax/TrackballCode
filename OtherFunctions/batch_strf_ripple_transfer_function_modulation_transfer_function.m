...
% batch_strf_ripple_transfer_function_modulation_transfer_function - compute all strf parameters,
% such as energy, phase-locking index, svd
% singular values, in one big process.
%
% batch_strf_ripple_transfer_function_modulation_transfer_function(sitenum)
%
...
for k = 1:length(stim)
   ...
   for i = 1:length(dsite)
      ...
      site = dsite(i).name;
      filename = [site '\*-' stim{k} '-*-strf-params.mat'];
      dfile = dir(filename);
      ...
      if ( ~isempty(dfile) )
         ...
         if ( strcmp(stim{k}, 'dmr1') | strcmp(stim{k}, 'dmr2') | strcmp(stim{k}, 'rn1') )
            ...
            outfile = [dfile.name(1:indstrfcmb-1) 'rtf-params']

            load([site '\' infile]);

            if ( exist('params')==1 )
 
               rtf_params = strf_ripple_transfer_function_modulation_transfer_function(params);
