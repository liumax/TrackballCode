...
%This model is based on the work of Steven Jacques (see Jacques, 1998;
%Jacques, 2003; Jacques, 2010)
%Inputs:
...
%
%Written by Joseph M. Stujenske, February 2014
...
if ~exist('dstep','var') || isempty(dstep)
    dr=.01; %discretion parameter for space in mm
else
    dr=dstep;
end
...
nlaunched=nphotons;
dz=dr; %discretion parameter for space in mm
...
while 1
    %choose step size for photons
    s=-log(rand(1,nphotons))/(absorption+scattering);
    
    %move photons in the x,y, and z direction by the step size s
    cors=cors+repmat(s,3,1).*dirs;
    
    %out of bounds photons are killed:
    outofbounds=sqrt(sum(cors(1:2,:).^2))>=rmax+dr | cors(3,:)>=zmax+dz | cors(3,:)<zmin;
    w(outofbounds)=0;
    
    %put out of bounds photons "inbounds" to avoid errors later on
    cors(:,w==0)=[zeros(2,sum(w==0));ones(1,sum(w==0))*zcenter];
    rcors=sqrt(sum(cors(1:2,:).^2));
    %photons weight is stored in the catcher matrix:
    ...
    %reduce weight of the photons by what was absorbed:
    w=w*(scattering/(absorption+scattering));
    
    %kill photons that are within rmax and rmax+dr and zmax and zmax+dz
    outofbounds2=(rcors)>=rmax | cors(3,:)>=zmax;
    w(outofbounds2)=0;
    
    %change direction of the photons:
    if g>0 && g<=1
    ...
    %photons very close to being vertical are treated as being
    %vertical:
    ...
    %roulette to see if photons with low weight die while preserving
    ...
    %terminate the simulation if all of the photons have no weight:
    if all(w==0)
        break
    end
    ...
    if nlaunched>=max(newplotthreshold,nphotons+1)
        ...
        %Estimate Time to Destroy the Rest of the Photons
