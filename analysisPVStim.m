...
function [s] = analysisPVStim(fileName);
...
toggleROCLoco = 0; %toggle for tuning on/off ROC analysis locomotion
toggleROCFiring = 0; %toggle for turning ROC for firing rates
...
s.Parameters.BaselineBin = [-5 0]; %time in seconds.
% s.Parameters.LFPWindow = [-1 1];
s.Parameters.PreWindow = -5;%seconds that the code will look before zero point for binning spike responses
s.Parameters.PostWindow = [1,4]; %alternative window for 
...
%for scrambling firing
alignWindows = [-5,0;-4,0;0,5;1,5;5,10];
scrambleReps = 10000;
...
for i = 1:numUnits
    ...
    prePostStore = zeros(totalTrialNum,6);
    preInd = find(histBinVector > s.Parameters.PreWindow,1,'first');
    preOne = find(histBinVector > -1,1,'first');
    zeroInd = find(histBinVector > 0,1,'first');
    laserInd = find(histBinVector > s.LowDur,1,'first');
    altInd1 = find(histBinVector > s.Parameters.PostWindow(1),1,'first');
    altInd2 = find(histBinVector > s.Parameters.PostWindow(2),1,'first');
    postInd = length(histBinVector);
    for k = 1:totalTrialNum
        prePostStore(k,1) = sum(fullHistHolder(preInd:zeroInd,k))/(-s.Parameters.PreWindow);
        prePostStore(k,2) = sum(fullHistHolder(zeroInd:laserInd,k))/(s.LowDur);
        prePostStore(k,3) = sum(fullHistHolder(laserInd:postInd,k))/(-s.Parameters.PreWindow);
        prePostStore(k,4) = sum(fullHistHolder(preOne:zeroInd,k));
        prePostStore(k,5) = sum(fullHistHolder(altInd1:altInd2,k))/(s.Parameters.PostWindow(2) - s.Parameters.PostWindow(1));
    end
    ...
    if toggleROCFiring == 1
        [meanStore,stdStore] = functionFiringScrambleResample(spikeTimes,alignTimes,alignWindows,scrambleReps);
        s.(desigNames{i}).MeanDistribution = meanStore;
    end
end
...
subplot(3,3,6)
hist((s.SumPlot.LaserPeriod-s.SumPlot.BaselineRates)./(s.SumPlot.LaserPeriod+s.SumPlot.BaselineRates))
xlim([-1 1])
