...
function [s] = analysisPVStimHiLowLaser(fileName);
...
for i = 1:length(firstFind)
    ...
    plot([0 modInd1(findOrder)],[firstArray(i,1) firstArray(i,1)],'b','LineWidth',2)
...
for i = 1:length(firstFind)
    findOrder = find(s.SortedPeakWaveOrder == firstFind(i));
    plot([0 modInd2(findOrder)],[firstArray(i,1) firstArray(i,1)],'g--','LineWidth',2)
...
for i = 1:length(secondFind)
    ...
    plot([0 modInd1(findOrder)],[secondArray(i,1) secondArray(i,1)],'b','LineWidth',2)
...
for i = 1:length(secondFind)
    findOrder = find(s.SortedPeakWaveOrder == secondFind(i));
    plot([0 modInd2(findOrder)],[secondArray(i,1) secondArray(i,1)],'g--','LineWidth',2)
    plot(modInd2(findOrder),secondArray(i,1),'g.')
    if ismember(findOrder,pvs)
        plot(modInd2(findOrder),secondArray(i,1),'r.')
    end
end
xlim([-1 1])
