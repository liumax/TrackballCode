function [output,allPeaks] = functionWavePropertyExtraction(averageWaves,averageRate);
...
waveInfo = zeros(3,size(averageWaves,2));
...
for waveCount = 1:size(averageWaves,2)
    ...
    for peakCount = 1:size(pks,1) 
        ...
        if isempty(leftMark)
            peaksPos(peakCount,3) = 2*abs(rightMark-peaksPos(peakCount,2));
        elseif isempty(rightMark)
            peaksPos(peakCount,3) = 2*abs(leftMark-peaksPos(peakCount,2));
        elseif ~isempty(leftMark) & ~isempty(rightMark)
            peaksPos(peakCount,3) = rightMark-leftMark;
        else
            error(strcat('PROBLEM WITH HALF WIDTH DETECTION FOR WAVE',num2str(waveCount)))
        end
    end
    ...
    if size(pksneg,1) > 0
        ...
        for peakCount = 1:size(pksneg,1) 
            ...
            %this code below is meant to extrapolate half-widths if one of
            %them cannot be detected. It doubles the distance from the
            %other side. 
            if isempty(leftMark)
                peaksNeg(peakCount,3) = 2*abs(rightMark-peaksNeg(peakCount,2));
            elseif isempty(rightMark)
                peaksNeg(peakCount,3) = 2*abs(leftMark-peaksNeg(peakCount,2));
            elseif ~isempty(leftMark) & ~isempty(rightMark)
    ...
    elseif peakNums > 4;
        ...
        hold on
        plot(allPeaks(:,2),allPeaks(:,1),'r*');
        hold off
        title(strcat('More than three peaks detected ',num2str(peakNums)));
    end

end
    waveInfo(3,:) = averageRate;
