function [data] = readTrodesExtractedDataFile(filename)  
...
if (~isempty(endHeaderLoc))
    ...
    for l=1:length(headerLines)
       ...
       if (~isempty(colonLoc))
           tmpFieldName = lower(regexprep(headerLines{l}(1:colonLoc-1),'[^\w'']','')); 
           tmpFieldVal = strtrim(headerLines{l}(colonLoc+1:end));
