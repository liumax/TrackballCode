...
%     end
    ...
    findWidths = find((s.NonLaserOverall.PosWidths(:,:,tarWin))>sigWin);
    ...
    for j = 1:length(keepList)
        ...
        fullMaster(fullCounter,10) = s.NonLaserOverall.PosWidths(1,keepList(j),tarWin);
        fullMaster(fullCounter,11) = s.LaserOverall.PosWidths(1,keepList(j),tarWin);
...
%         fullMaster(fullCounter,16) = s.LaserOverall.PosWidths(2,keepList(j),tarWin);
        ...
        tempData = squeeze(s.(s.DesignationName{keepList(j)}).BinSigVals(1+s.SoundData.WhiteNoise:end,:,tarWin));
        ...
        tempData = squeeze(s.(s.DesignationName{keepList(j)}).BinSigValsLaser(1+s.SoundData.WhiteNoise:end,:,tarWin));
        minData = min(tempData);
        minData = find(minData < 0.05,1,'first');
        if minData
