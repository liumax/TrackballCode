...
for i = 1:numFiles
    ...
    for j = 1:numCells
        ...
        %significance
        binValBigStore{bigMasterInd + j - 1} = s.(desigName{j}).BinTone;
        sigValBigStore{bigMasterInd + j - 1} = s.(desigName{j}).BinSigVals(:,:,2);
        latMapBigStore{bigMasterInd + j - 1} = s.(desigName{j}).LatencyMap;
...
%     end
    ...
    widthStore(bigMasterInd:bigMasterInd + numUnits - 1) = squeeze(s.PosWidths(end,:,3));
    %store BFs
