...
%% 180302 eliminating manual toggle.  toggleTuneSelect = 0; %1 means you want to select tuning manually, 0 means no selection.
...
%% Removing time points beyond time filter. 
...
for i = 1:numUnits
    ...
    %to all sounds.    
    %180302 first, find only the biggest amplitude, will use these for
    %calculation. 
    setPoints = alignTimes(master(:,3) == max(uniqueDBs));
    ...
    genSigSaver(i,1) = generalResponseHist.SigSpikePos;
    genSigSaver(i,2) = generalResponseHist.SigSpikeNeg;
