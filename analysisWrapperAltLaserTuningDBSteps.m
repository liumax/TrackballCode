...
sigVals = 5; %minimum significant responses to be considered for analysis 
tarWin = 2; %target window of analysis. 1 = fast, 2 = tone, 3 = gen. 
tarSig = 1; %significance level, 0.05, 0.01, 0.001 (1,2,3)
sigWin = 1; %minimum width for analysis. 
...
for i = 1:numFiles
    ...
    load(targetFiles{i})
    disp(strcat('Loading-',targetFiles{i}))
...
%     end
    ...
    findSigs = find(sum(s.NonLaserOverall.PosTots(:,:,tarSig))>sigVals);
    keepList(findSigs) = keepList(findSigs) * 2;
    ...
    keepList = find(keepList == 2);
    disp(strcat('Keeping ',num2str(length(keepList)),' out of', num2str(numUnits)))
