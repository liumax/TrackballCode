...
scQtUserData.trigDur = str2num(answer{i});i=i+1;
...
scQtUserData.maxTrials = 100000; %this is for calculating ITIs. Placeholder of obscenely large size to avoid issues of running out of times.

pause(0.2);

sendScQtControlMessage(['disp(''Mouse ID:', scQtUserData.mouseID,''')']);
sendScQtControlMessage(['disp(''Drive ID:', scQtUserData.mouseID,''')']);
sendScQtControlMessage(['disp(''Trigger Duration:', num2str(scQtUserData.trigDur),''')']);
...
sendScQtControlMessage(['trigDur=',num2str(scQtUserData.trigDur)]);
