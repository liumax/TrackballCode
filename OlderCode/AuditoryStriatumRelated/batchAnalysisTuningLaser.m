...
structureName = genvarname(fileName(1:periodFinder-1));
[matclustStruct] = analysisBasicTuning(fileName);
fullMaster.(structureName) = matclustStruct;
clearvars matclustStruct
[matclustStruct] = analysisBasicLaserStimDIO(fileName);
fullMaster.(structureName).LaserData = matclustStruct;
cd(masterFolder)
end

saveName = strcat('FullTuningAndLaserAnalysis',date,'.mat');
save (fullfile(pwd,saveName),'fullMaster');