function wf = getWaveFormsPkChan(gwfparams)
% function wf = getWaveFormsPkChan(gwfparams)
%
% TL modified to only extract waveforms from channel specified in
% gwfparams.clusterChannel
%
...
%
% gwfparams.clusterChannel                 % which channel to extract for each cluster, 1-based
%
% % OUTPUT
...
waveForms = nan(numUnits,gwfparams.nWf,wfNSamples);
waveFormsMean = nan(numUnits,wfNSamples);
for curUnitInd=1:numUnits
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        waveForms(curUnitInd,curSpikeTime,:) = mmf.Data.x(chMap(gwfparams.clusterChannel(curUnitInd)),spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(1):spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end));
    end
