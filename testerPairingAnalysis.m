...
timesTuningSecond = [DIO1True(signalHolder(2,3)),DIO1True(signalHolder(1,4))];
...
timesPairing = DIO1True(signalHolder(2,2)+1:signalHolder(1,3)-1);
timesTuningSecond = DIO1True(signalHolder(2,3)+1:signalHolder(1,4)-1);
...
[s] = functionPairingTuning(s,truncatedNames,...
    spikeNames{1},soundNames{1},soundNames{1},params.rasterWindow,params.histBin,params.clusterWindow,...
    params.clims1,params.rpvTime,trodesDesignation,fileName); 
...
%next, pull tuning information. DOES NOT GRAPH
[s] = functionPairingTuning(s,truncatedNames,...
    spikeNames{2},soundNames{2},soundNames{2},params.rasterWindow,params.histBin,params.clusterWindow,...
    params.clims1,params.rpvTime,trodesDesignation,fileName); 
...
[s] = functionPairingTonePresentSoundExtraction(s,...
    soundNames{3},soundFile); 
%process TTLs (since these are not just unitary TTLs signaling tone onset.
[s] = functionPairingPairedTTLAdjust(s,soundNames{5},...
    soundFile,soundNames{5});
...
[s] = functionPairingToneAnalysis(s,truncatedNames,...
    spikeNames{5},soundNames{5},soundNames{5},params.rasterWindow,histBinLong,params.clusterWindow,...
    params.clims1,params.rpvTime,trodesDesignation,fileName); 

%NOW I NEED TO PLOT EVERYTHING IN A WAY THAT MAKES SENSE
[s] = functionPairingMasterPlot(numTrodes,s,...
    truncatedNames,params.rpvTime,params.clusterWindow,params.rasterWindow,params.histBin,...
    params.clims1,fileName,trodesDesignation,soundNames);
